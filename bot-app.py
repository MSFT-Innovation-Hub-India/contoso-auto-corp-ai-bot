...
# create the AzureChatOpenAI object
llm = AzureChatOpenAI(
...
# create a new thread id for the session
thread_id = str(uuid.uuid4())

# provide bootstrapping information to the Bot, which is the Customer Name in the conversation
# this is an existing user in the database
config = {
    "configurable": {
...
# these are the actions that the Bot has access to it
tools = [
    store_service_feedback,
    fetch_customer_information,
    get_available_service_slots,
...
# define the state of the Bot. It will contain the user context, messages, and the current datetime
class State(TypedDict):
    messages: Annotated[list[AnyMessage], add_messages]
    customer_info: str
    current_datetime: str

# The LLM powered assistant that will help the user with the service scheduling
class Assistant:
...
# define the prompt template for the service scheduling assistant
...
## Uncomment this to print the graph for the bot
